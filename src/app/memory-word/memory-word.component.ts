...
@Component({
  selector: 'app-memory-word',
  standalone: true,
  imports: [CommonModule, RouterLink, ProfileModalComponent, LoginModalComponent, ConfittiComponent],
  templateUrl: './memory-word.component.html',
  styleUrl: './memory-word.component.css'
})
export class MemoryWordComponent implements OnInit, OnDestroy {
  gridSize = 4;
  maxTries = 5;
  ...
  async loadGameData(): Promise<void> {
    ...
    // if (insta_user_ig) {
    //   localStorage.setItem('user_inst_ID', insta_user_ig);
    // }
    if (!contestId) {
  ...
  selectTile(tile: Tile): void {
    ...
    if (this.selections.length === 2) {
      ...
      if (this.selections[0].value === this.selections[1].value) {
        setTimeout(() => {
          this.selections.forEach((t) => (t.matched = true));
          this.matches++;
          this.selections = [];
          this.boardLocked = false;
        ...
        // Only increment moves on a wrong match
        // this.moves++;

        setTimeout(() => {
          this.selections.forEach((t) => (t.flipped = false));
          this.selections = [];
          this.boardLocked = false;
        }, 1000);
      }
