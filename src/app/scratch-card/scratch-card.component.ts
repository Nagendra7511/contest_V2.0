...
export class ScratchCardComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  private sendResultToApi(offer: any, voucher: string, contestId: string, isWinner: boolean): void {
    if (!this.userId || !contestId) return;

    const key = `resultSent_${this.userId}_${contestId}`;
    ...
    // if (isWinner && offer) {
    //   result.voucher_assigned = voucher;
    //   result.expiry_date = offer.expiryDate;
    // }
  ...
  // }
   async onGameFinished() {
    if (!this.userId || !this.contest?.contest_id) {
      console.error('Missing userId or contestId – cannot save participation.');
      return;
    }
    const success = await this.supabaseService.playContest(this.userId, this.contest.contest_id);
    if (success) {
